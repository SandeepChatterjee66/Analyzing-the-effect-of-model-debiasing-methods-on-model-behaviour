...
output_path = 'resources'  # Directory where output resources will be saved
log_soft = F.log_softmax  # Function used for computing the log softmax of input tensors
tokenizer_dir = "./tokenizer"  # Directory where tokenizer-related files are stored
model_dir = "./model"  # Directory where model files are stored
config_dir = "./config"  # Directory where model configuration files are stored

# Maximum length of input sequences (suitable for most datasets)
MAX_LEN = 512  
...
# Hyperparameters or coefficients used in the model or loss function
alpha1 = 0.2  
alpha2 = 2  
alpha3 = 0.9  

# Number of output labels in the classification task
num_labels = 3  



class BiasModel(nn.Module):
    """A bias model for generating logits based on attention mechanisms.
    ...
    Attributes:
        ...
        classifier (nn.Linear): Classifier for generating logits.
    ...
    def forward(self, bert_hidden_layer_input):
        ...
        Args:
            bert_hidden_layer_input (torch.Tensor): Input tensor representing hidden layers from BERT.

        Returns:
            torch.Tensor: Logits generated by the bias model.
...
class MainModel(BertPreTrainedModel):
    """Custom model based on BERT for multi-task learning.

    This model incorporates a BERT-based architecture along with a bias model
    for an attention mechanism and additional layers for final classification.

    Args:
        config (PretrainedConfig): Configuration for the BERT model.
        loss_fn (Optional[Callable]): Loss function for the main task (default: None).

    Attributes:
        num_labels (int): Number of output labels for the classification tasks.
        loss_fn (Callable): Loss function for the main task.
        bert (BertModel): Pre-trained BERT model.
        bias_model (BiasModel): Model for the attention mechanism.
        hidden_layer (Linear): Linear layer for transformation before classification.
        classifier (Linear): Final linear layer for classification.
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, labels, device):
        ...
        Args:
            ...
            labels (Tensor): True labels for the main task.
            device (str): Device to run the computation ('cpu' or 'cuda').

        Returns:
            Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]: Tuple containing main loss, main predictions,
            bias loss, bias predictions, and forget loss.
    ...
    def set_bias_grad(self, requires_grad):
        """Set whether bias model parameters require gradients.
...
def train(model, dataloader, optimizer, device):
    ...
    Args:
        ...
        dataloader (DataLoader): DataLoader object containing the training dataset.
    ...
    for idx, batch in enumerate(dataloader):
        ...
        main_loss, main_pred, bias_loss, bias_pred, forget_loss = model(input_ids=input_ids, attention_mask=mask, token_type_ids=token_type_ids, labels=targets, device=device)
...
def valid(model, dataloader, device):
    ...
    Args:
        model (MainModel): The model to be evaluated.
        dataloader (DataLoader): DataLoader object containing the validation dataset.
        device (str): Device to run the computation ('cpu' or 'cuda').
    ...
    for batch in dataloader:
        ...
        loss_main, main_prob, loss_bias, _, _ = model(input_ids=input_ids, attention_mask=mask, token_type_ids=token_type_ids, labels=targets, device=device)
