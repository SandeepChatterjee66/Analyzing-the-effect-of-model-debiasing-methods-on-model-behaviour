...
do
    ...
    do
        ...
        do
            ...
            mkdir -p "./results_on_train/$MODEL"
            # Run KNN Out-of-Distribution Detection experiments
            python3 ood.py --train_dataset_name $TRAIN \
                                   --test_dataset_name $TEST \
                                   --input_file_train ../resources/Embeddings/$MODEL/$TRAIN/$TRAIN\_train.txt \
                                   --train_model_name $MODEL \
                                   --input_file_val ../resources/Embeddings/$MODEL/$TRAIN/$TRAIN\_devel.txt \
                                   --input_file2 ../resources/Embeddings/$MODEL/$TRAIN/$TEST\_test.txt \
                                   --pred_dir $PRED_DIR \
                                   --test_groundtruth ../resources/Embeddings/$TEST\_test_groundtruth.txt \
                                   --train_groundtruth ../resources/Embeddings/$TRAIN\_train_groundtruth.txt \
                                   --percent 95
        done
    done
done
