...
from Entity_classification.generate_embeddings.LfF.train import MyModel
from Entity_classification.generate_embeddings.LfF.data_loader import load_data
from Entity_classification.generate_embeddings.LfF.cross_eval import cross_eval
...
def inference(dataset_name,model, dataloader, tokenizer, device, id2label):
    ...
    for idx, batch in enumerate(tqdm(dataloader, ncols=100)):
        ...
        with torch.no_grad():
            ...
            text_file = dataset_name+dataset_name+'embeddings.txt'
            np.savetxt(text_file,z_l.cpu().numpy())
...
def main():
    ...
    # print(config)
    # print("new_length of tokenizer",len(tokenizer))
    model = MyModel.from_pretrained(input_model_path, config = config)
    #You have not specified another important parameter special_tokens_fix=0
    ...
    if(args.train_dataset_name == args.test_dataset_name):
        ...
        test_accuracy, pred_lst, weights, prob_lst = inference(args.test_dataset_name,model, test_dataloader, tokenizer, device, id2label)
