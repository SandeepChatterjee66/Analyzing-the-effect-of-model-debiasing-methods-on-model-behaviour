...
from Entity_classification.generate_embeddings.LfF.data_loader import load_mapping
...
class MainModel(BertPreTrainedModel):
    ...
    def features(self,input_ids, attention_mask):
        """
        Computes the features using the model.
...
def generate_embeddings(model, dataloader, device, output_file_path):
    """
    Generates embeddings using the model and saves them to a text file.
...
def main():
    """
    Main function for generating embeddings using a trained model.
    """
    start = time.time()
